...
/** Same URL deeplink scheme as confirm email (io.biblenow.authapp) so the app opens reliably. */
const VERIFICATION_COMPLETE_DEEP_LINK_BASE = 'io.biblenow.authapp://verification-complete';

export default function VerificationCompletePage() {
  const [deepLink, setDeepLink] = useState<string>(VERIFICATION_COMPLETE_DEEP_LINK_BASE);
  ...
  useEffect(() => {
    ...
    const link = sessionId
      ? `${VERIFICATION_COMPLETE_DEEP_LINK_BASE}?session_id=${encodeURIComponent(sessionId)}`
      : VERIFICATION_COMPLETE_DEEP_LINK_BASE;
    setDeepLink(link);
